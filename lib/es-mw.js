...
// ES
curl -X GET 'http://localhost:9200/entity/_search?pretty'
curl -X GET 'http://localhost:9200/entity/_mapping?pretty'
curl -XDELETE 'http://localhost:9200/entity/user'
*/
...
function getEsHits(es_res, hit_index) {
  // Results from a genified method is returned as an array
  var hits = es_res[0].hits.hits
...
function *getUserData(client, user_email) {
  ...
  // TODO! Improve search. Use "es-filter"?
  ...
  debug('getUserData', user_email, res)
...
function *updateUser(client, user_id, body_doc) {
  var index = 'entity'
  var type = 'user'

  debug('updateUser', user_id, body_doc)
...
exports.setCtxMerchant = function(client, hostname) {
  ...
  return function*setCtxMerchant(next) {
    var res
    var hit
    ...
    if (!hostname || hostname === 'localhost') {
      debug('setCtxMerchant', 'Invalid hostname', hostname)
    ...
    debug('setCtxMerchant', this.merchant_data)
...
exports.registerUser = function(client) {
  ...
  return function*registerUser(next) {
    ...
    debug('registerUser', client.unique.exists)

    res = yield client.unique.exists('email', body.email)
    if (res) {
    ...
    yield client.unique.create('email', body.email)
...
exports.setCtxUserRec = function(client) {
  ...
  return function*setCtxUserRec(next) {
    ...
    res = yield getUserData(client, user_email)
...
exports.makeLoginToken = function(client, expires_after) {
  ...
  return function *makeLoginToken(next) {
    ...
    res = yield updateUser(client, this.user_data.id, {
...
exports.authorizeUser = function(client) {
  ...
  return function*authorizeUser(next) {
    ...
    assert(!this.user_data.login_token, 'No Login Token for user. Please request a new token')
    assert(!this.user_data.login_token.revoked_at, 'Login Token is revoked. Please request a new token')
...
exports.returnMerchant = function(client) {

  return function*returnMerchant(next) {
...
exports.returnPages = function(client) {
  ...
  return function*returnPages(next) {
    ...
    getEsHits(res).map(function(hit) {
