...
exports.verifyLoginToken = function() {

  return function*verifyLoginToken(next) {

    var body = this.request.body

    debug('verifyLoginToken', 'body', body)

    // Decode and validate body login_token
    try {
      var login_token = utils.decodeJwtToken(body.token)
    } catch(err) {
      throw new UserError('Invalid token: '+body.token)
    }
    assert(utils.isNotExpired(login_token), 'Token expired')
